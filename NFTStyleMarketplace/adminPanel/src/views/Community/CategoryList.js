...
import { getAllcategory , deletecategory } from './../../actions/community';
 toast.configure();
let toasterOption = {
  position: "top-right",
  autoClose: 5000,
...
export default function EnhancedTable() {
    ...
    {
        if(id!='') {
            window.location="/DigitalCollectibleAdmin/Communitycategoryedit/"+id;
        }
    ...
    const getAll = async (search = {}) => {
        ...
        //console.lo('result===================', res);
        setUser(res.result.userValue);
        // //console.lo('result of')
        }
    }
    ...
    const handleDelete = async (iid) => {
        try {
            ...
            //toastAlert('success', res.result.data.message, 'currency')
             toast.success(res.result.data.message,toasterOption);
                ...
                >
                    <Typography className={classesSearch.title} variant="h6" id="tableTitle" component="div">
                        ...
                        <div className={classes.flexHeader}>
                            <Link to={'/Communitycategoryadd'} className={classes.btnPrimary}>Add Category</Link>
                        </div>
