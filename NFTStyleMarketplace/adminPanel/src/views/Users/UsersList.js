...
import { getuserslist, deleteToken, getuserdaily, getusermonthly, getusercustom, getcsvdata } from '../../actions/users';
...
const columns = [
...

  {
    ...
    <div>{record.image !="" && <img width="25" height="25" src={`${config.Back_Url}/images/${record.id}/${record.image}`}/>}</div>
...

  {
    ...
    cell: record =>
    // //console.lo(record.date,"date")
    <div>{record.coverimage !="" && <img width="25" height="25" src={`${config.Back_Url}/images/coverimages/${record.id}/${record.coverimage}`}/>}</div>
      ...
      return (
        <Fragment>
         ...
         {/* <button
            ...
            {record._id!=""&&  <Link to={`viewUser/${record._id}`}>
...
async function deleteR(id) {
  if (id != '') {
    ...
    setTimeout(
      () => window.location.href = "/nimdaelytstfn/UsersList",
      1000);
...
export default function EnhancedTable() {
  ...
  const [userdet1, setUser1] = useState([]);
  ...
  const getUserslist1 = async () => {
    var test = await getuserslist();
    console.log("DFdsdsdsdfsd"+JSON.stringify(test))
    setUser1(test.userValue);
  }
