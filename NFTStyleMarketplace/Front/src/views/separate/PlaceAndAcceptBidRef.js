...
async function BidApply_ApproveCall() {
  ...
  if (!config.providercon) {
    ...
    var web3 = new Web3(config.providercon);
    var currAddr = (UserAccountAddr != "")? UserAccountAddr : await getCurAddr();
    if (!currAddr) {
      toast.warning("OOPS!..connect Your Wallet", toasterOption);
    }
...
async function AcceptBid_Proceed() {
    var curAddr = props.UserAccountAddr;
    if (config.providercon) {
      var web3 = new Web3(config.providercon);
      var handle=null;
      var receipt=null;
...
useImperativeHandle(
  ...
  () => ({
    ...
    if(props.providerss){
      var web3=new Web3(props.providerss)
      var BuyOwnerDetail={
        tokenOwner:props.UserAccountAddr,
        tokenCounts:curBid_val.tokenCounts
      }
      ...
      if(curBid_val && curBid_val.tokenBidAmt) {
        ...
        var ServiceFee_val = (toMid/100000000)*((Service_Fee/config.decimalvalues)*1000000)
        var YouWillGet_Val = (toMid - ServiceFee_val);
        //console.log("hbebfunwbufn",TotAmt,toMid,ServiceFee_val,YouWillGet_Val)
              ...
              <div class="modal-body px-0 pt-0">
                <form className="px-4 bid_form">
                  ...
                  <div className="col-12 col-sm-6 text-sm-right">
                      <p className="buy_desc_sm_bold">{props.UserAccountBal} {config.currencySymbol}</p>
                  </div>
                  </div>
                  <div className="row pb-3">
                  <div className="col-12 col-sm-6">
                  ...
                  <div className="col-12 col-sm-6 text-sm-right">
                      <p className="buy_desc_sm_bold">{((props.Service_Fee)/(10**18))}% <span>{coinname}</span></p>
                  </div>
                  </div>
                  <div className="row pb-3">
                  <div className="col-12 col-sm-6">
      ...
      <div class="modal fade primary_modal" id="accept_modal" tabindex="-1" role="dialog" aria-labelledby="accept_modalCenteredLabel" aria-hidden="true">
          ...
          <div class="modal-content">
              ...
              <div className="row mx-0 pb-3">
                  ...
                  <p className="buy_desc_sm_bold">{((props.Service_Fee)/(10**18))}%</p>
                  </div>
              </div>
              <div className="row mx-0 pb-3">
                  <div className="col-12 col-sm-6 px-4">
