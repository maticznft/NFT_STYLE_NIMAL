...
export const PurchaseNowRef = forwardRef((props, ref) => {
    ...
    const PriceCalculate = async (data = {}) => {
        ...
        console.log("dhfnjsngjrdngrktnij",totfee,serfee,toMid,(serfee/1000000),(toMid/1000000))
        //var tot2cont = web3.utils.toWei(String(totfee/1e6))
        var tot2cont = null
        var aprrove = null
        if (NewTokenDecimal == 18) {
            tot2cont = web3.utils.toWei(String(Number((Number(totfee)))))
            aprrove = web3.utils.toWei(String(Number((Number(totfee)))))
        }
        else {
            tot2cont = web3.utils.toWei(String(Number((Number(totfee)))))
    ...
    async function FormSubmit_StepOne() {
        ...
        if (config.providercon) {
            ...
            if (
                ...
                try {
                    if (item.type == 721) {
                        await bidvalue.methods.approve(config.singleContract, ApproveWei.toString()).send({ from: Wallet_Details.Accounts })
                    ...
                    if (receipt != null) {
                        ...
                        if (receipt.status == true) {
                            ...
                            var postData = {
                                ...
                                coin:item.tokenowners_current.CoinName
                            }
