...
useEffect(() => {
  setmodalopen(true)
  getInit(Wallet_Details.UserAccountAddr);
  getProfileData();
...
var currAddr = isEmpty(UserAccountAddr)?Wallet_Details.UserAccountAddr : UserAccountAddr;
var payload = {}
// if (UserAccountAddr != '') {
...
const handleFile = async (e) => {
  ...
  let addr = isEmpty(paramAddress) ? Wallet_Details.UserAccountAddr : paramAddress;
  var reqdata = {
    file,
    currAddr: addr,
  }
   ...
   <GridContainer className="mx-0">
    ...
    (MyItemAccountAddr_Details.curraddress == Wallet_Details.UserAccountAddr)
     ? <Link className="theme-btn flex_btn" to="/edit-profile">Edit Profile</Link> 
      : 
      <button className={'theme-btn flex_btn'} onClick={() => followFun(Wallet_Details.UserAccountAddr)}> 
       {followButton}
      </button>
     }
