...
export const BurnField = async (req, res) => {  
  try {
    ...
    let checkUser = await Token.findOne({ "tokenCounts": reqBody.tokenCounts,"contractAddress":reqBody.contractAddress,"type":reqBody.type });
    ...
    if(checkUser){
      var ch3= await Token.findOneAndUpdate({ "tokenCounts": reqBody.tokenCounts,"contractAddress":reqBody.contractAddress}
       ,{$set:{
         "balance":((checkUser.balance)-parseInt(reqBody.balance)),
         "burnToken":(checkUser.burnToken)+parseInt(reqBody.balance)
        }
      },
      {new:true}
      );
...
export const getpurchasetokenlist = async (req, res) => {
  try{
    var userData=await ActivityDB
    .aggregate([
      {
        $match:{       
          action:{         
      ...
      },
      {
        ...
        {$unwind:{
            ...
            //      $gt:['$tokenowners_current.balance',0]}
            ...
            preserveNullAndEmptyArrays: true,
                  ...
                  preserveNullAndEmptyArrays: true,
                      ...
                      action:1,
                      activity:1,
                      from:   {$cond: { if: { $ne: [ "$fromUser.name", "" ] }, then: "$fromUser.name", else: "$fromUser.curraddress" }} ,
                      to:  {$cond: { if: { $ne: [ "$toUser.name", "" ] }, then: "$toUser.name", else: "$toUser.curraddress"  }},
                      tokenCounts:1,
                      created:1,
                      amount:1,
                      currencySymbol:1,
                    
                      // tokenowners_current_balance:
                      // tokenowners_current:{
                        tokenowners_current_balance:'$tokenowners_current.balance',
                        tokenowners_current_CoinName:'$tokenowners_current.CoinName',
                        tokenowners_current_tokenOwner:'$tokenowners_current.tokenOwner',
                        tokenowners_current_contractAddress:'$tokenowners_current.contractAddress',
                        tokenowners_current_type:'$tokenowners_current.type',
                        tokenowners_current_status:'$tokenowners_current.status',
                        tokenowners_current_tokenPrice:'$tokenowners_current.tokenPrice',
                          // },
                      // toUser:{
                        toUser_name:'$toUser.name',
                        toUser_curraddress:'$toUser.curraddress',

                      // }
                      // ,
                      // fromUser:{
                        fromUser_name:'$fromUser.name',
                        fromUser_curraddress:'$fromUser.curraddress',
                        
                      // },
                      // tokenNme:{
                        tokenNme_tokenName:'$tokenNme.tokenName'
                      // }
