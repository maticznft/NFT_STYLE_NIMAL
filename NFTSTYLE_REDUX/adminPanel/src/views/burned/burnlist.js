...
import { getpromotiontokenlist, setPromotions,getpromotion,deleteToken, getuserdaily, getusermonthly, getusercustom, getcsvdata,getburntokenlist } from '../../actions/users';
...
export default function BURNLIST() {
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: false,
      cell:val=>
      <div>{(val.tokenOwnerDb.tokenOwner)}</div>
    ...
    },
    {
      ...
      sortable: false,
      cell:val=>
      <div>{String(val.tokenOwnerDb.burnToken)}</div>
    ...
    },
    {
      ...
      cell: record =>
        // //console.lo(record.date,"date")
        <div><p>{moment(record.tokenOwnerDb.timestamp).format('MMMM,Do YYYY, hh:mm A')}</p></div>
    },
  ...
  const getTokenlist = async () => {
    var test = await getburntokenlist();
