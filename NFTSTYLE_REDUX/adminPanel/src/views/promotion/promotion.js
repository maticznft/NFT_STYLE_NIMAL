...
export default function PROMOTIONLIST() {
  ...
  const columns = [
  ...
  
    {
      ...
      sortable: false,
      cell: record => {
        var   getVal=userdet.filter((rec)=>(rec.swapPrice)==1)
        setSelected1(getVal.length)
        //console.log"Selected12",getVal.length)
          return (
          <Fragment>
                    
            {
              record.swapPrice==0 ?
                ...
                :
                <Button className="secondary-btn" onClick={()=>removepromote(record)}>
                  remove promotion
                </Button>
            }
                
                {/* <p>{(record.swapPrice),Selected12}</p> */}
                 {/* <input type="radio" value={record.tokenCounts} name={record.tokenCounts}
               checked={(Selected12 == record.tokenCounts) }
               onChange={()=>{onValueChange(record.tokenCounts)}}
                 /> */}
  ...
  const promote =async(data)=>{
    //console.log"Selected12",Selected12)
    if(Selected12 >=2){
      toast.warning("Please select only two tokens for promotion")
  ...
  const getTokenlist = async () => {
    var test = await getpromotiontokenlist();
